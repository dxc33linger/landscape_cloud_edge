...
num_epoch1 = int(args.epoch_edge * 0.5)
...
method.initialization(args.lr, args.epoch * 0.5, args.weight_decay)
...
# method.initial_single_network(init_weights = True)
# method.initialization(args.lr, int(args.epoch*0.7), args.weight_decay)
...
#
# 	if epoch >= 0.9*int(args.epoch) and test_acc_0[-1] > best_acc_0:
# 			best_acc_0 = test_acc_0[-1]


test_task_accu.append(best_acc_0)
test_acc_0_end.append(best_acc_0)
torch.save(method.net.state_dict(), '../../results/{0}/model_afterT{1}_Accu{2:.4f}.pt'.format(args.model, 0, best_acc_0))
...
for task_id in range(1, total_task):
	...
	_, test_mix_full = get_dataset_cifar(all_list, 0)
	...
	method.initialization(args.lr*0.1, int(num_epoch1), args.weight_decay)
	best_acc_mix = 0.0
	for epoch in range(num_epoch1):
		...
		logging.info('train_acc {0:.4f} \n\n\n'.format(train_acc[-1]))
		if epoch >= int(0.9*num_epoch1) and test_acc_mix[-1] > best_acc_mix:
			best_acc_mix = test_acc_mix[-1]


	# logging.info("Current Task is {} : Train task 1 w/ injecting memory iteratively ====================".format(task_id))
	# method.initialization(args.lr*0.1, int(num_epoch2*0.3), args.weight_decay)
	# best_acc_mix = 0.0
	...
	# 	if epoch >= int(0.9*num_epoch2) and test_acc_mix[-1] > best_acc_mix:
	# 		best_acc_mix = test_acc_mix[-1]
	...
	# method.initialization(args.lr*0.01, int(num_epoch3*0.3), args.weight_decay)
	# best_acc_mix = 0
	...
	# 	test_acc_current.append(method.test(test_edge))
	# 	test_acc_mix.append(method.test(test_mix_full))
	# 	if epoch >= int(0.9*num_epoch3) and test_acc_mix[-1] > best_acc_mix:
	# 		best_acc_mix = test_acc_mix[-1]
	# 	logging.info('>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>> Single-head T0 testing accu is : {:.4f}'.format( test_acc_0[-1]))
	# 	logging.info('>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>> Single-head current testing accu is : {:.4f}'.format( test_acc_current[-1]))
	# 	logging.info('>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>> Single-head mixed all tasks testing accu is : {:.4f}'.format( test_acc_mix[-1]))
	...
	torch.save(method.net.state_dict(), '../../results/{0}/model_afterT{1}_Accu{2:.4f}.pt'.format(args.model, task_id, best_acc_mix))
	#
...
plt.savefig('../../results/{}/incremental_curve_T{}_{:.4f}.png'.format(args.model, task_id, best_acc_mix))
...
plt.savefig('../../results/{}/PSTmain_learning_curve_model_{}_acc{:.4f}.png'.format(args.model, args.model, best_acc_mix))

param = count_parameters_in_MB(method.net)
logging.info('Param:%s',param)
scio.savemat('../../results/{}/PSTmain_model{}_acc{:.4f}.mat'.format(args.model, args.model, best_acc_mix),
             {'train_acc':train_acc, 'test_acc_0':test_acc_0,'test_acc_current':test_acc_current,
              'test_acc_mix':test_acc_mix,
            ...
            'classes_per_task': args.classes_per_task, 'test_acc_0_end':test_acc_0_end,
            'test_task_accu':test_task_accu, 'weight_decay': args.weight_decay,  'score': args.score,
