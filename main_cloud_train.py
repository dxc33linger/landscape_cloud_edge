...
logging.info('Freeze {} weight of model {}'.format(task_division[0]/10, method.save_folder))
current_mask_list, current_threshold_dict, mask_dict_pre, maskR_dict_pre, current_taylor_dict = method.sensitivity_rank_taylor_filter(task_division[0]/10)
...
scio.savemat('../../results/cloud_training_model_{}.mat'.format(args.model), {'train_acc':train_acc, 'test_acc':test_acc, 'cloud_list':cloud_list, 'current_edge_list':current_edge_list, 'avg_test':avg_test})
...
plt.xticks(np.arange(0, len(test_acc), step=10))
